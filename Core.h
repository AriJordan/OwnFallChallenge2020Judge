...
public:
    ...
    const ULL BITS_PER_ITEM = 6;
    const ULL FIRST_24_BITS = 0xffffff; // TODO: test valid_inventory() with this
    const ULL SIXTH_BITS = 0x820820;
    const ULL FIFTH_BITS = 0x410410;
    ///////////////////////////////////////////////

    ///////////////own spell values////////////////
    const map<vector<int>, double> LEARNS_MAP =
    ...
    const int TOTAL_STATES = 3000;
    //const int MAX_STATES_PER_D = 400; // Number of different states per distance
    const double EXPECTED_SCORE_PER_TURN = 3.1;
    const double SHORT_TERM_DIS = 10.0;
    const double CONST_SHORT_TERM_DIS = 1.0;
    const double SPELL_VALUE_MUL = 0.012;
    const int EXPECTED_SPELLS_LEARNED = 8;
    const double SPELLS_LEARNED_EXP = 1.6;
    const double VALUE_DENSITY_MUL = 4.0;
    const double OWN_VALUE_MUL = 1.15;
    const double SPELLS_SIMILARITY_VALUE_MUL = 3.5;
    const double RECIPES_SIMILARITY_VALUE_MUL = 3.0;
    const double SPELL_CNT_DIFF_MUL = 0.8;
    const double CASTABLE_BONUS_MUL = EXPECTED_SCORE_PER_TURN;
    const double COULD_BREW_MUL = 1.8;
    const double COULD_BREW_BONUS[6] = { 0.0, 1.8 * COULD_BREW_MUL, 3.0 * COULD_BREW_MUL, 3.5 * COULD_BREW_MUL, 3.75 * COULD_BREW_MUL, 4.0 * COULD_BREW_MUL };
    ...
    const double BLUE_VAL = 0.9; // value of tier 0 item // 1.1 before
    ...
    bool IS_LOCAL = true;
#elif defined OWN_JUDGE
    const bool DEBUG = false; // Print cerr messages
    bool IS_LOCAL = true;
#else
    const bool DEBUG = true; // Print cerr messages
    bool IS_LOCAL = false;
#endif
    const int LOCAL_TIME_BONUS = 200000; // local addition of milliseconds
    //////////////////////////////////////

    std::chrono::steady_clock::time_point STTMP = chrono::steady_clock::now();
    ...
    inline double adj_score_per_turn(double new_value_d, int dist) {
        return new_value_d / (dist + 0.001) / (SHORT_TERM_DIS * 1.0 / dist + CONST_SHORT_TERM_DIS);
    }
...
public:
    Core(double PARAMETER) : SPELLS_SIMILARITY_VALUE_MUL(PARAMETER), glob_m_spells(0), glob_o_spells(0) {
    ...
    std::string execute_turn(std::string player_input) {
        cin << player_input;
        glob_step_num++;
        ...
        if (glob_m_brewed > 6 || glob_o_brewed > 6) {
            std::cerr << "Error, too manybrewed\n";
        }
        assert(glob_m_brewed <= 6);
        assert(glob_o_brewed <= 6);
