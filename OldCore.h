...
const double SPELL_VALUE_MUL = 0.017;
const int EXPECTED_SPELLS_LEARNED = 11;
const double VALUE_DENSITY_MUL = 4.0;
const double SPELLS_SIMILARITY_VALUE_MUL = 3.3;
const double RECIPES_SIMILARITY_VALUE_MUL = 3.5;
const double SPELL_CNT_DIFF_MUL = 0.4;
const double CASTABLE_BONUS_MUL = EXPECTED_SCORE_PER_TURN;
const double COULD_BREW_BONUS[6] = { 0.0, 2.0, 3.0, 3.5, 3.75, 4.0 };
...
const double BLUE_VAL = 1.1; // value of tier 0 item // 1.1 before
...
double get_state_score_difference(ULL state, const vector<action>& recipes, const vector<double>& learns_value, int spells_cnt, double startvalue, int dist, int mode) {
    ...
    double learned_val = state_learned_value(state, learns_value) * EXPECTED_SPELLS_LEARNED / learned_cnt;
...
inline vector<double> own_learns_val(const vector<action>& learns) {
    ...
    for (int l = 0; l < 6; l++) {
        own_learns_value.push_back(LEARNS_MAP.at(learns.at(l).deltas));
    }
...
inline double adj_score_per_turn(double new_value_d, int dist) {
    return new_value_d / (dist + 0.001) / (8.0 * 1.0 / dist + 1.0);
}
